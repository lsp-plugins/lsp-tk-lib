...
#include <lsp-plug.in/tk-old/widgets/LSPGraph.h>
#include <time.h>
...
namespace lsp
{
    namespace tk
    {
        ...
        ws::ISurface *LSPGraph::get_canvas(ws::ISurface *s, ssize_t w, ssize_t h, const Color & color)
        {
            // Check surface
            if (pCanvas != NULL)
            {
        ...
        status_t LSPGraph::on_mouse_down(const ws::event_t *e)
        {
        ...
        void LSPGraph::draw(ws::ISurface *s)
        {
            // Prepare palette
            Color color(sColor);
