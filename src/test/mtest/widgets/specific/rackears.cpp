...
MTEST_BEGIN("tk.widgets.specific", rackears)
    ...
    MTEST_MAIN
    {
        ...
        MTEST_ASSERT(wnd->title()->set_raw("Test rack ears") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("rackears_test") == STATUS_OK);
...
//        wnd->padding()->set_all(16);
...

        {
            ...
            MTEST_ASSERT(ra = new tk::RackEars(dpy));
            MTEST_ASSERT(init_widget(ra, vh, id.get_ascii()) == STATUS_OK);
            ...
            MTEST_ASSERT(init_widget(ra, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(ra));
            MTEST_ASSERT(grid->add(ra) == STATUS_OK);
            ...
            MTEST_ASSERT(init_widget(vw, vh, id.get_ascii()) == STATUS_OK);
