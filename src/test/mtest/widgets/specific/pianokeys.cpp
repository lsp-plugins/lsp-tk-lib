...
MTEST_BEGIN("tk.widgets.specific", pianokeys)
    ...
    static status_t slot_mouse_scroll(tk::Widget *sender, void *ptr, void *data)
    {
        ...
        const ws::event_t *ev = static_cast<const ws::event_t *>(data);
        tk::PianoKeys *pk = tk::widget_cast<tk::PianoKeys>(sender);
        if (pk != NULL)
        {
            if (ev->nCode == ws::MCD_UP)
            {
                if (pk->min_note()->get() > 0)
                {
                    pk->min_note()->sub(1);
                    pk->max_note()->sub(1);
                }
    ...
    MTEST_MAIN
    {
        ...
        tk::Grid *grid = NULL;
        tk::PianoKeys *pk = NULL;
        size_t vid = 0;

        // Initialize window
        MTEST_ASSERT(init_widget(wnd, vh, "window") == STATUS_OK);
        MTEST_ASSERT(wnd->title()->set_raw("Test piano keys") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("pianokeys_test") == STATUS_OK);
