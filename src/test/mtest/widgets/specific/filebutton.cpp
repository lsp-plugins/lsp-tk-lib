...
MTEST_BEGIN("tk.widgets.specific", filebutton)
    ...
    static status_t slot_mouse_click(tk::Widget *sender, void *ptr, void *data)
    {
        ...
        {
            tk::FileButton *btn = tk::widget_cast<tk::FileButton>(sender);
            if (btn != NULL)
                btn->active()->toggle();
        }
    ...
    MTEST_MAIN
    {
        ...
        MTEST_ASSERT(wnd->title()->set_raw("Test file button") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("filebutton_test") == STATUS_OK);
...
//        wnd->padding()->set_all(16);
        ...
        // Create popup menu
        MTEST_ASSERT(pm = new tk::Menu(dpy));
        MTEST_ASSERT(init_widget(pm, vh, "menu") == STATUS_OK);
        MTEST_ASSERT(widgets.push(pm));

        MTEST_ASSERT(mi = new tk::MenuItem(dpy));
        MTEST_ASSERT(init_widget(mi, vh, "menuitem") == STATUS_OK);
        MTEST_ASSERT(widgets.push(mi));
        MTEST_ASSERT(pm->add(mi) == STATUS_OK);

        mi->text()->set_raw("Popup Menu");

        {
            ...
            // Create fraction
            MTEST_ASSERT(id.fmt_ascii("filebutton-%d", int(vid++)));
            MTEST_ASSERT(fb = new tk::FileButton(dpy));
            MTEST_ASSERT(init_widget(fb, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(fb));
            MTEST_ASSERT(grid->add(fb) == STATUS_OK);
            ...
            fb->text()->set("Save");
            fb->value()->set_all(0.6f, 0.0f, 1.0f);
            fb->popup()->set(pm);
            ...
            fb->text()->set("Some\nMultiline\nText");
            fb->value()->set_all(1.0f, 0.0f, 1.0f);
            fb->popup()->set(pm);
            ...
            fb->text_list()->append()->set_raw("WWWWWWWWWWW\n\n");
            fb->text()->set("Some\nMultiline\nText");
            fb->text_layout()->set(-1.0f, -1.0f);
            fb->value()->set_all(0.4f, 0.0f, 1.0f);
            fb->popup()->set(pm);
        }
