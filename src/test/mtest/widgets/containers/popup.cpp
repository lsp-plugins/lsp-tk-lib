...
MTEST_BEGIN("tk.widgets.containers", popup)
    ...
    static status_t slot_mouse_up(tk::Widget *sender, void *ptr, void *data)
    {
        ...
        if (!h->wnd->visibility()->get())
        {
            ws::rectangle_t r;
            sender->get_screen_rectangle(&r);
    ...
    status_t init_widget(tk::Widget *w, lltl::parray<handler_t> &vh, tk::PopupWindow *wnd, const char *label)
    {
        status_t res = w->init();
        if (res != STATUS_OK)
            return res;
...
//        wnd->padding()->set_all(16);
...

        {
            ...
            for (size_t i=0; i<2; ++i)
            {
                ...
                // Create filler
                MTEST_ASSERT(id.fmt_ascii("fill-%d", int(i+4)));
                MTEST_ASSERT(vw = new tk::Void(dpy));
                MTEST_ASSERT(init_widget(vw, vh, NULL, id.get_ascii()) == STATUS_OK);
                MTEST_ASSERT(widgets.push(vw));
                vw->size_constraints()->set_min(16, 16);
