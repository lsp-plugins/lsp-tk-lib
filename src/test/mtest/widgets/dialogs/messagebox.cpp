...
MTEST_BEGIN("tk.widgets.dialogs", messagebox)
    ...
    static status_t slot_ok_submitted(tk::Widget *sender, void *ptr, void *data)
    {
        ...
        sender->display()->queue_destroy(h->mbox);
        h->mbox     = NULL;
        return STATUS_OK;
    }
    ...
    static status_t slot_btn_submit(tk::Widget *sender, void *ptr, void *data)
    {
        handler_t *h = static_cast<handler_t *>(ptr);
        h->test->printf("BUTTON SUBMITTED: %s\n", h->label);

        tk::MessageBox *mbox = new tk::MessageBox(sender->display());
        mbox->init();
        mbox->heading()->set_raw("Heading");
        mbox->message()->set("Button '{@name}' has been clicked");
    ...
    status_t init_widget(tk::Widget *w, lltl::parray<handler_t> &vh, const char *label)
    {
        ...
        if (btn != NULL)
        {
            if (hid >= 0) hid = w->slots()->bind(tk::SLOT_SUBMIT, slot_btn_submit, h);
        }
    ...
    MTEST_MAIN
    {
...

        {
            ...
            for (ssize_t x=0; x<4; ++x)
            {
                ...
                MTEST_ASSERT(id.fmt_ascii("button-%d", x));
