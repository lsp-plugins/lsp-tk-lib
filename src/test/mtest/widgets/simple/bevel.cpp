...
namespace
{
    static const float angles[] = { 0.0f, 30.0f, 45.0f, 60.0f };
}

MTEST_BEGIN("tk.widgets.simple", bevel)
    ...
    MTEST_MAIN
    {
        ...
        MTEST_ASSERT(wnd->title()->set_raw("Test bevel") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("bevel_test") == STATUS_OK);
...
//            for (size_t x=6; x<7; ++x)
            {
                MTEST_ASSERT(id.fmt_ascii("bevel-%d", x));
                MTEST_ASSERT(bv = new tk::Bevel(dpy));
                MTEST_ASSERT(init_widget(bv, vh, id.get_ascii()) == STATUS_OK);
                MTEST_ASSERT(widgets.push(bv));
                MTEST_ASSERT(grid->add(bv) == STATUS_OK);
