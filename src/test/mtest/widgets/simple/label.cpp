...
MTEST_BEGIN("tk.widgets.simple", label)
    ...
    MTEST_MAIN
    {
        ...
        MTEST_ASSERT(wnd->title()->set_raw("Test label") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("label_test") == STATUS_OK);
...

        {
            ...
            for (ssize_t y=0; y<5; ++y)
                for (ssize_t x=0; x<5; ++x)
                {
                    ...
                    MTEST_ASSERT(id.fmt_ascii("label-%d-%d", x, y));
                    MTEST_ASSERT(label = new tk::Label(dpy));
                    MTEST_ASSERT(init_widget(label, vh, id.get_ascii()) == STATUS_OK);
