...
MTEST_BEGIN("tk.widgets.simple", rangeslider)
    ...
    MTEST_MAIN
    {
        ...
        tk::Grid *grid = NULL;
        tk::RangeSlider *rs = NULL;

        // Initialize window
        MTEST_ASSERT(init_widget(wnd, vh, "window") == STATUS_OK);
        MTEST_ASSERT(wnd->title()->set_raw("Test range slider") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("range_slider_test") == STATUS_OK);
...
//        wnd->padding()->set_all(16);
...

        {
            ...
            MTEST_ASSERT(id.fmt_ascii("hslider-0"));
            MTEST_ASSERT(rs = new tk::RangeSlider(dpy));
            MTEST_ASSERT(init_widget(rs, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(rs));
            MTEST_ASSERT(grid->add(rs, 1, 8) == STATUS_OK);

            rs->button_color()->set_rgb24(next_color(col));
            rs->button_border_color()->set(rs->button_color());
            rs->button_width()->set(14);
            rs->button_pointer()->set(ws::MP_HSIZE);
            rs->balance_color()->set_rgb24(0xffff00);
            ...
            for (ssize_t x=0; x<8; ++x)
            {
                ...
                MTEST_ASSERT(id.fmt_ascii("vslider-%d-0", x));
                MTEST_ASSERT(rs = new tk::RangeSlider(dpy));
                MTEST_ASSERT(init_widget(rs, vh, id.get_ascii()) == STATUS_OK);
                MTEST_ASSERT(widgets.push(rs));
                MTEST_ASSERT(grid->add(rs) == STATUS_OK);
                ...
                rs->button_pointer()->set(ws::MP_VSIZE);
                rs->balance_color()->set_rgb24(0xffff00);
                rs->inactive_balance_color()->set_rgb24(0xcccccc);
                rs->balance_color_custom()->set(true);
            ...
            {
                // Create alignment widget
                MTEST_ASSERT(id.fmt_ascii("vslider-%d-1", x));
                MTEST_ASSERT(rs = new tk::RangeSlider(dpy));
                MTEST_ASSERT(init_widget(rs, vh, id.get_ascii()) == STATUS_OK);
                MTEST_ASSERT(widgets.push(rs));
                MTEST_ASSERT(grid->add(rs) == STATUS_OK);
            ...
            // Create horizontal fader
            MTEST_ASSERT(id.fmt_ascii("hslider-1"));
            MTEST_ASSERT(rs = new tk::RangeSlider(dpy));
            MTEST_ASSERT(init_widget(rs, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(rs));
            MTEST_ASSERT(grid->add(rs, 1, 8) == STATUS_OK);
