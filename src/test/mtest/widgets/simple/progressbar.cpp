...
MTEST_BEGIN("tk.widgets.simple", progressbar)
    ...
    static status_t slot_mouse_click(tk::Widget *sender, void *ptr, void *data)
    {
        ...
        tk::ProgressBar *pb = tk::widget_ptrcast<tk::ProgressBar>(sender);
        if (pb != NULL)
        {
            ws::event_t *ev = static_cast<ws::event_t *>(data);
            if (ev->nCode == ws::MCB_LEFT)
                pb->value()->add(0.05f, true);
            else if (ev->nCode == ws::MCB_RIGHT)
                pb->value()->sub(0.05f, true);
        }
    ...
    MTEST_MAIN
    {
        ...
        tk::Grid *grid = NULL;
        tk::ProgressBar *pb = NULL;

        // Initialize window
        MTEST_ASSERT(init_widget(wnd, vh, "window") == STATUS_OK);
        MTEST_ASSERT(wnd->title()->set_raw("Test progressbar") == STATUS_OK);
        MTEST_ASSERT(wnd->role()->set_raw("progressbar_test") == STATUS_OK);
...
//        wnd->padding()->set_all(16);
...

        {
            ...
            // Create progress bar
            MTEST_ASSERT(id.fmt_ascii("progressbar-0"));
            MTEST_ASSERT(pb = new tk::ProgressBar(dpy));
            MTEST_ASSERT(init_widget(pb, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(pb));
            MTEST_ASSERT(grid->add(pb) == STATUS_OK);
            pb->border_color()->set_rgb24(0xff0000);
            pb->show_text()->set(false);

            // Create progress bar
            MTEST_ASSERT(id.fmt_ascii("progressbar-1"));
            MTEST_ASSERT(pb = new tk::ProgressBar(dpy));
            MTEST_ASSERT(init_widget(pb, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(pb));
            MTEST_ASSERT(grid->add(pb) == STATUS_OK);

            pb->bg_color()->set_rgb24(0x1b1c22);
            pb->border_color()->set_rgb24(0xdae0ff);
            pb->border_gap_color()->set_rgb24(0x1b1c22);
            pb->color()->set_rgb24(0x00c0ff);
            pb->inv_color()->set_rgb24(0xdae0ff);
            pb->text()->set_raw("Some text");
            pb->show_text()->set(true);

            // Create progress bar
            MTEST_ASSERT(id.fmt_ascii("progressbar-2"));
            MTEST_ASSERT(pb = new tk::ProgressBar(dpy));
            MTEST_ASSERT(init_widget(pb, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(pb));
            MTEST_ASSERT(grid->add(pb) == STATUS_OK);

            pb->constraints()->set(0, 0, -1, -1);
            pb->allocation()->set_fill(true);
            pb->border_radius()->set(10);
            pb->text()->set_raw("Some very long text with line separator\nand multiline text.");

            // Create progress bar
            MTEST_ASSERT(id.fmt_ascii("progressbar-3"));
            MTEST_ASSERT(pb = new tk::ProgressBar(dpy));
            MTEST_ASSERT(init_widget(pb, vh, id.get_ascii()) == STATUS_OK);
            MTEST_ASSERT(widgets.push(pb));
            MTEST_ASSERT(grid->add(pb) == STATUS_OK);
            pb->border_color()->set_rgb24(0x000088);
            pb->constraints()->set(64, 32, -1, -1);
            pb->border_gap_color()->set_rgb24(0xcccccc);
            pb->color()->set_rgb24(0x0044cc);
            pb->show_text()->set(false);
            pb->value()->set(1.0f);
        }
